var bt_tools = {
    ...
    table: function (config) {
        ...
        ReaderTable.prototype = {
            ...
            $refresh_table_list: function (load) {
                var _that = this,
                    loadT;
                if (load) loadT = bt.load('正在获取列表数据,请稍候...');
                    ...
                    case 'group':
                        ...
                        $.each(item.group, function (index, items) {
                            ...
                            if (items.template) {
                                ...
                                _html += '<a href="javascript:;" class="btlink ' + className + '" ' + (items.hide ? 'style="display:none;"' : '') + ' title="' + (items.tips || items.title) + '">' + items.title + '</a>';
                            }
                    ...
                    if (active.callback) {
                        ...
                        if (!active.confirm) {
                            if (active.confirmVerify) {
                                bt.show_confirm('批量' + active.title, '批量' + active.title + '，该操作可能会存在风险，是否继续？', request)
                            } else {
                                bt.confirm({
                                    title: '批量' + active.title,
    ...
    form: function (config) {
        ...
        ReaderForm.prototype = {
            ...
            $reader_content: function (callback) {
                ...
                event_list['input_checked_' + that.random] = {
                    'focus': {
                        type: 'input_checked',
                        onEvent: false,
                ...
                $.each(data, function (key, item) {
                    ...
                    switch (item.type) {
                        ...
                        case 'number': // 数字
                            ...
                            html += '<input type="' + item.type + '"' + attribute + ' ' + (item.icon ? 'id="' + _event + '"' : '') + ' class="bt-input-' + (item.type != 'select_path' ? item.type : 'text') + ' mr10 ' + (item.label ? 'form_middle' : '') + _that.$verify(item.class, 'class') + '"' + _that.$verify(style, 'style') + ' />';
                            if (item.icon) {
                        ...
                        (function (items, key) {
                            if (items.onEvent === false) {
                                ...
                                that.element.on(keys != 'event' ? keys : 'click', items.children ? items.children : childNode, function (ev) {
                                    var form = that.$get_form_element(),
                                        value = that.$get_form_value(),
     ...
     */
    load: function (title) {
        var loadT = layer.msg('正在' + title + '，请稍候...', {
            icon: 16,
            time: 0,
            shade: .3
        });
    ...
    msg: function (param1, param2) {
        ...
        if (typeof param1 === "object") {
            if (typeof param1.status === "boolean") {
                msg = param1.msg, config = {
                    icon: param1.status ? 1 : 2
                };
            }
        }
    ...
    send: function (param1, param2, param3, param4, param5, param6) {
        ...
        $.ajax({
            ...
            error: function (er) {
                ...
                layer.msg('服务响应错误：' + er.status + '</br>错误信息：' + er.statusText + '</br>接口信息：' + params.url + '</br>接口参数：' + JSON.stringify(params.data), {
                    icon: 2,
                    time: 0,
                    closeBtn: 2
                });
