var bt_file = {
    ...
    file_header:{'file_checkbox': 40,'file_name':'auto','file_accept':120,'file_size': 90,'file_mtime':145,'file_ps':'auto','file_operation':300,'file_tr':0,'file_list_header':0},
    ...
    event_bind: function(){
        ...
        $('.filePage').on('click','#file_all_size',function(e){
            if(that.file_path === '/'){
                bt.confirm({title:'警告',msg:'当前目录为系统根目录（/）,执行获取文件大小将占用<span class="bt_danger">大量服务器IO</span>，是否继续？'},function(){
    ...
    set_file_table_width:function(){
        ...
        $.each(this.file_header,function(key,item){
            if(item == 'auto'){ 
                ...
                width += item;
                css += '.'+key+'{width:'+ (key != 'file_operation'?item:item - 5)  +'px !important;}';
            }
        });
        ...
        if((this.file_list.length * tr_heigth) > $('.file_list_content').height()){
            ...
            other += '.file_td.file_operation{width:'+ (this.file_header['file_operation'] - this.scroll_width - 10) +'px !important;}';
            other += '.file_th.file_operation{padding-right:'+ (10 + this.scroll_width) +'px !important}';
        }else{
            file_header_width = file_header_width;
            console.log(file_header_width)
    ...
    */
    reader_file_list_content:function(data,callback){
        ...
        $.each(data,function(index,item){
            var _title = item.filename,only_id = bt.get_random(10),path = (that.file_path +'/'+ item.filename).replace('//','/'),is_compress = that.determine_file_type(item.ext,'compress')
            console.log(is_compress)
            ...
            _html += '<div class="file_tr" data-index="'+ index +'" data-filename="'+item.filename+'" '+(bt.get_cookie('rank') == 'icon'?'title="'+ path +'&#13;'+lan.files.file_size+':'+bt.format_size(item.size)+'&#13;'+lan.files.file_etime+':'+bt.format_data(item.mtime)+'&#13;'+lan.files.file_auth+':'+item.user+'&#13;'+lan.files.file_own+':'+item.root_level+'"':'')+'>'+
                ...
                '<div class="file_td file_name">'+
                    ...
                    '<span class="file_title file_'+ item.type +'_status" '+(bt.get_cookie('rank') == 'icon'?'':'title="' + path + '"')+'><i>'+ item.filename +'</i></span>'+ (item.caret?'<span class="iconfont icon-favorites" style="'+ (item.down_id != 0?'right:30px':'') +'" title="文件已收藏，点击取消"></span>':'') + (item.down_id != 0?'<span class="iconfont icon-share1" title="文件已分享，点击查看信息"></span>':'') +
                '</div>'+ 
                ...
                '<div class="file_td file_size"><span>'+ (item.type == 'dir'?'<a class="btlink folder_size" href="javascript:;" data-path="'+ path +'">点击计算</a>':bt.format_size(item.size)) +'</span></div>'+
                ...
                '</div></div>'+
            '</div>';
            // <div class="foo_menu"><span class="foo_menu_title">操作</span></div>
    ...
    */
    render_file_groud_menu: function (ev,el){
        var that = this,index = $(el).data('index'),_openTitle = '打开',data = that.file_list[index],config_group = [['open',_openTitle],['split',''],['download','下载'],['share','分享目录/文件'],['cancel_share','取消分享'],['favorites','收藏目录/文件'],['cancel_favorites','取消收藏'],['split',''],['dir_kill','目录查杀'],['authority','权限'],['split',''],['copy','复制'],['shear','剪切'],['rename','重命名'],['del','删除'],['split',''],['compress','创建压缩'],['unzip','解压'],['open_find_dir','打开文件所在目录']],compression = ['zip','rar','gz','war','tgz','bz2'],offsetNum = 0;
        // 文件类型判断
        ...
        layer.open({
            ...
            success: function () {
                $.post('/files?action=get_videos', { path: path }, function (rdata){
                    var video_list = '<table class="table table-hover" style="margin-bottom:0;"><thead style="display: none;"><tr><th style="word-break: break-all;word-wrap:break-word;width:165px;">文件名</th><th style="width:65px" style="text-align:right;">大小</th></tr></thead>',index = 0;
    ...
    */
    ...
    paste_file_or_dir:function(){
        ...
        if(that.file_path.indexOf(_paste) > -1){
            layer.msg('您不能将“'+_filename+'”粘贴到此位置，因为项目不能粘贴到自身。',{icon:0})
            return false;
        }
